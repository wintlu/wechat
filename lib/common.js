...
API.prototype.getAccessToken = function (callback) {
  var that = this;
  var url = this.prefix + 'gettoken?grant_type=client_credential&corpid=' + this.appid + '&corpsecret=' + this.appsecret;
...
API.prototype._createMenu = function (menu, callback) {
  var url = this.prefix + 'menu/create?access_token=' + this.token + '&agentid=' + this.agentid;
...
API.prototype._removeMenu = function (callback) {
  var url = this.prefix + 'menu/delete?access_token=' + this.token + '&agentid=' + this.agentid;
  urllib.request(url, {dataType: 'json'}, wrapper(callback));
};
